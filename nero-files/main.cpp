...
int SDL(Graph *graph, std::vector<Node *> forbiddenNodes, std::vector<std::vector<Node *> *> pathNodes)
{
  ...
  std::vector<std::vector<SDL_Rect> *> snapshots;
  // snapshots.push_back(new std::vector<SDL_Rect>);
  ...
  while (!quit)
  {
    ...
    while (SDL_PollEvent(&event))
    {
      ...
      case SDL_KEYDOWN:
        ...
        case SDLK_SPACE:
          ...
          std::ifstream fileReadSnapshots;
          std::string x1;
          std::string y1;
          std::string x2;
          std::string y2;
          ...
          while (std::getline(fileReadSnapshots, temp, ' '))
          {
            ...
            case 4:
              ...
              snapshots.back()->push_back(SDL_Rect{
                  .x = (std::stoi(x1) * CELL_SIZE) + xPadding,
                  .y = (std::stoi(y1) * CELL_SIZE) + yOffset + yPadding,
                  .w = CELL_SIZE,
                  .h = CELL_SIZE,
              });

              snapshots.back()->push_back(SDL_Rect{
                  .x = (std::stoi(x2) * CELL_SIZE) + xPadding,
                  .y = (std::stoi(y2) * CELL_SIZE) + yOffset + yPadding,
                  .w = CELL_SIZE,
                  .h = CELL_SIZE,
              });
    ...
    // }
    for (Node *node : pathNodes.back()[0])
    {
      // std::cout << "GGGGGGGGGGGG\n";
      std::cout << pathNodes.back()->size() << std::endl;
    }

    // for (SDL_Rect rect : snapshots[0][0])
    // {
...
int SDL(Graph *graph)
{
  ...
  std::vector<std::vector<Node *> *> pathNodes = graph->getRandomPathSnapshots();
  std::vector<Node *> selectedNodes;
  bool selectingNodesAllowed = false;
  ...
  while (!quit)
  {
    ...
    while (SDL_PollEvent(&event))
    {
      ...
      case SDL_KEYDOWN:
        ...
        case SDLK_SPACE:
          ...
          for (std::vector<Node *> *vec : pathNodes)
          {
            vec->clear();
          }
          snapshotNumber = 0;
          pathNodes.clear();
          // pathNodes = graph->getRandomPath();
          pathNodes = graph->getRandomPathSnapshots();
    ...
    if (pathNodes.size() > 0)
    {
      ...
      {
        for (int n = 0; n < pathNodes.at(s)->size(); n++)
        {
          if (s == snapshotNumber)
          {
            drawCell(renderer, pathNodes.at(s)->at(n), pathNodesColor, yOffset, xPadding, yPadding);
          }
          else
          {
            drawCell(renderer, pathNodes.at(s)->at(n), pathNodesPastColor, yOffset, xPadding, yPadding);
          }
        }
      }
