...
int const CELL_SIZE = 1;
int const MAP_SIZE_X = 50;
int const MAP_SIZE_Y = 50;

// TODO - I'm ignoring the last remaining space, shouldn't be a problem
int const X_NODES = int(MAP_SIZE_X / CELL_SIZE);
int const Y_NODES = int(MAP_SIZE_Y / CELL_SIZE);
...
{
  for (int x = 0; x < MAP_SIZE_X; x += CELL_SIZE)
  {
    for (int y = 0; y < MAP_SIZE_Y; y += CELL_SIZE)
    {
      ...
      if (x > 0)
      {
        graph.nodes[x][y]->addNeighbor(graph.nodes[x - CELL_SIZE][y]);
        graph.nodes[x - CELL_SIZE][y]->addNeighbor(graph.nodes[x][y]);
      };
      if (y > 0)
      {
        graph.nodes[x][y]->addNeighbor(graph.nodes[x][y - CELL_SIZE]);
        graph.nodes[x][y - CELL_SIZE]->addNeighbor(graph.nodes[x][y]);
      };
    };
  };
};
