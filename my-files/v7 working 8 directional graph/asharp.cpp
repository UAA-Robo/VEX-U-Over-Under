...
double const GOAL_START = 46.64;
double const GOAL_LENGTH = 93.77 - 46.64;
double const GOAL_WIDTH = 23.08;
...
// double const ROLLER_POINT_1 = 0 * CONVERSION_FACTOR;
// double const ROLLER_POINT_2 = 23.08 * CONVERSION_FACTOR;
// double const ROLLER_POINT_3 = 117.33 * CONVERSION_FACTOR;
// double const ROLLER_POINT_4 = 140.41 * CONVERSION_FACTOR;
...
int main()
{
  ...
  vector<Node *> path = AStar(graph.nodes[0][0], graph.nodes[1][1], h);
  ...
  for (int x = 0; x < X_NODES; x++)
  {
    ...
    {
      cout << "Node (" << graph.nodes[x][y]->x << ", " << graph.nodes[x][y]->y << ")\n";
      cout << "Neighbors " << graph.nodes[x][y]->neighbors.size() << endl;
      // for (Node *neightbor : graph.nodes[x][y]->neighbors)
      // {
